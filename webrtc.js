...
async function initiateCall(contactName, isVideo) {
  ...
  try {
    ...
    alert(i18nT('micCamError') + e.message);
  }
}
...
async function answerCall(contactName, offerData, isVideo) {
  ...
  try {
    ...
    alert(i18nT('answerCallError') + e.message);
  }
}
...
async function handleCallSignal(payload) {
  ...
  if (msg.type === 'answer' && rtc.isCaller) {
    ...
    document.getElementById('callSt').textContent = i18nT('inCallTimer');
    startCallTimer();
  }
...
function showIncomingCall(from, offerData) {
  var name = from;
  if (confirm('📞 ' + name + ' ' + i18nT('incomingCallConfirm'))) {
...
function showVoiceCallScreen() {
  ...
  document.getElementById('callNm').textContent = G.chat || i18nT('callFallbackName');
  document.getElementById('callSt').textContent = i18nT('connecting');
  show('callscr');
}
